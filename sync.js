...
// Configuración compartida del hogar. IndexedDB conserva una copia para uso
// offline, pero Supabase es la fuente de verdad entre dispositivos.
async function syncSharedSettingsWithSupabase() {
    if (!supabaseClient) return;
    ...
    if (data) {
        await saveSetting('category_budgets', data.value || {});
        return;
    }

    const localBudgets = await getSetting('category_budgets', {});
    const { error: insertError } = await supabaseClient
        .from('app_settings')
        .upsert({ key: 'category_budgets', value: localBudgets }, { onConflict: 'key' });
...
async function syncWithSupabase() {
    ...
    try {
        ...
        try {
            await syncSharedSettingsWithSupabase();
        } catch (settingsError) {
            // La sincronización de movimientos no debe fallar por una
            // configuración pendiente de migrar en Supabase.
            console.warn('No se pudo sincronizar la configuración compartida:', settingsError.message);
        }
